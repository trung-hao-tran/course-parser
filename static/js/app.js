document.addEventListener('alpine:init', () => {
  Alpine.data('courseApp', () => ({
    ...
    selectFilter(field, value) {
      // Check if filter already exists in selected filters
      const existingIndex = this.selectedFilters.findIndex((f) => f.field === field && f.value === value);
    ...
    isFilterSelected(field, value) {
      return this.selectedFilters.some((f) => f.field === field && f.value === value);
    },

    // Apply all selected filters
    applyFilters() {
      // Replace active filters with selected filters
    ...
    removeFilter(field, value) {
      const index = this.activeFilters.findIndex((f) => f.field === field && f.value === value);

      if (index >= 0) {
        this.activeFilters.splice(index, 1);

        // Start fresh with all courses when removing a filter
        this.filteredCourses = [...this.courses];

        // Then reapply remaining filters
        if (this.searchQuery && this.searchQuery.length >= 1) {
          this.searchTable();
        } else {
          this.filterCourses();
    ...
    processTeacherData(teacherMap, teacherName, course) {
      ...
      teacherData.subjects.add(course.course_symbol);
