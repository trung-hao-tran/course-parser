...
def get_courses():
    try:
        ...
        if lecture_halls_exists:
            logger.debug("Lecture halls table exists, joining with courses")
            query = '''
                SELECT 
                    c.course_symbol, 
                    c.course_datetime, 
                    ...
                    c.hall
                FROM courses c
                LEFT JOIN lecture_halls lh ON 
                    date(c.course_datetime) = date(lh.hall_datetime) AND
                    c.class = lh.class
            '''
        else:
            logger.debug("Using basic query without lecture hall join")
